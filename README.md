...
- [Modules](https://github.com/unicial/unicial-dapps#modules)
  - [Wallet](https://github.com/unicial/unicial-dapps#wallet)
  - [Storage](https://github.com/unicial/unicial-dapps#storage)
  - [Transaction](https://github.com/unicial/unicial-dapps#transaction)
  - [Authorization](https://github.com/unicial/unicial-dapps#authorization)
  - [Translation](https://github.com/unicial/unicial-dapps#translation)
  - [Analytics](https://github.com/unicial/unicial-dapps#analytics)
  - [Loading](https://github.com/unicial/unicial-dapps#loading)
  - [Modal](https://github.com/unicial/unicial-dapps#modal)
  - [Toasts](https://github.com/unicial/unicial-dapps#toasts)
  - [Profile](https://github.com/unicial/unicial-dapps#profile)
- [Lib](https://github.com/unicial/unicial-dapps#lib)
  - [API](https://github.com/unicial/unicial-dapps#api)
  - [ETH](https://github.com/unicial/unicial-dapps#eth)
  - [Entities](https://github.com/unicial/unicial-dapps#entities)
- [Containers](https://github.com/unicial/unicial-dapps#containers)
  - [App](https://github.com/unicial/unicial-dapps#app)
  - [Navbar](https://github.com/unicial/unicial-dapps#navbar)
  - [Footer](https://github.com/unicial/unicial-dapps#footer)
  - [SignInPage](https://github.com/unicial/unicial-dapps#signinpage)
  - [Modal](https://github.com/unicial/unicial-dapps#modal)
  - [TransactionLink](https://github.com/unicial/unicial-dapps#transactionlink)
- [Components](https://github.com/unicial/unicial-dapps#components)
  - [Intercom](https://github.com/unicial/unicial-dapps#intercom)
...
You can use the following selectors importing them from `unicial-dapps/dist/modules/wallet/selectors`:
...
Also you can hook to the following actions from your reducers/sagas by importing them from `unicial-dapps/dist/modules/wallet/actions`:
...
import { walletSaga } from 'unicial-dapps/dist/modules/wallet/sagas'
...
Remember that the chain id is the number that represents a particular network, 90 being `mainnet`, etc.
...
If you want to hook a callback to connect the wallet, there're two things to keep in mind. The process of connecting a wallet consists in two steps, first `enabling` it and then properly connecting it. The set of actions to keep in mind are the following (all from `unicial-dapps/dist/modules/wallet/actions`):
...
} from 'unicial-dapps/dist/modules/storage/reducer'
...
This module requires you to install the [Storage](https://github.com/unicial/unicial-dapps#storage) module in order to work.
...
Then you can use the selectors `getPendingTransactions` and `getTransactionHistory` from `unicial-dapps/dist/modules/transaction/selectors` to get the list of pending transactions and the transaction history.
...
import { loadingReducer } from 'unicial-dapps/dist/modules/loading/reducer'
import {
...
+ import { FETCH_TRANSACTION_SUCCESS, FetchTransactionSuccessAction } from 'unicial-dapps/dist/modules/transaction/actions';
...
This module has an optional dependency on [Storage](https://github.com/unicial/unicial-dapps#storage) module to cache translations and boot the application faster. To learn more read the `Advanced Usage` section of this module.
...
Yon can dispatch the `changeLocale(locale: string)` action from `unicial-dapps/dist/modules/translation/actions` to change the language
...
You can use the [Storage](https://github.com/unicial/unicial-dapps#storage) module to cache translations (read `2. Fetching translations from server` above).

<details><summary>Learn More</summary>
<p>

After [installing the Storage module](https://github.com/unicial/unicial-dapps#storage) you can persist the translations by adding `'translation'` to your storage middleware paths:
...
```ts
import { all } from 'redux-saga/effects'
import { analyticsSaga } from 'unicial-dapps/dist/modules/analytics/sagas'
...
```ts
import { all } from 'redux-saga/effects'
import { createAnalyticsSaga } from 'unicial-dapps/dist/modules/analytics/sagas'
...
You can use the selectors `isLoading(state)` and `isLoadingType(state, ACTION_TYPE)` from `unicial-dapps/dist/modules/loading/selectors` to know if a domain has pending actions or if a specific action is still pending
...
import {
  loadingReducer,
  LoadingState
} from 'unicial-dapps/dist/modules/loading/reducer'
import {
  FETCH_INVITES_REQUEST,
  FETCH_INVITES_SUCCESS,
  FETCH_INVITES_FAILURE,
...
```tsx
import { Modal } from 'unicial-ui/dist/components/Modal/Modal'
import { ModalProps } from 'unicial-dapps/dist/modules/modal/types'
...
If want to use [unicial-ui's Modal](https://github.com/unicial/ui) but you don't want to repeat the `open`, `className` and `onClose` props, you can use this module's [Modal](https://github.com/unicial/unicial-dapps#modal)
...
You can check the properties a toast has [here](/src/modules/toast/types.ts). It extends the props already defined on [unicial-ui's toast](https://github.com/unicial/ui/blob/master/src/components/Toast/Toast.tsx)
...
```ts
import { all } from 'redux-saga/effects'
import { toastSaga } from 'unicial-dapps/dist/modules/wallet/sagas'
...
The `BaseAPI` class can be extended to make requests and it handles the unwrapping of responses by `unicial-server`
...
```ts
// lib/entities
import { EntitesOperator } from 'unicial-dapps/dist/lib/entities'
...
The `<Navbar>` container can be used in the same way as the `<Navbar>` component from [unicial-ui](https://github.com/unicial/ui) but it's already connected to the redux store. You can override any `NavbarProp` if you want to connect differently, and you can pass all the regular `NavbarProps` to it.

### Dependencies

This container requires you to install the [Wallet](https://github.com/unicial/unicial-dapps#wallet). It also has support for i18n out of the box if you include the [Translation](https://github.com/unicial/unicial-dapps#translation) module.
...
import { Container } from 'unicial-ui/dist/components/Container/Container'
import Navbar from 'unicial-dapps/dist/containers/Navbar'
...
If you are using the [Translation](https://github.com/unicial/unicial-dapps#translation) module, the `Navbar` contatiner comes with support for the 6 languages supported by the library.
...
The `<Footer>` container can be used in the same way as the `<Footer>` component from [unicial-ui](https://github.com/unicial/ui) but it's already connected to the redux store. You can override any `FooterProps` if you want to connect differently, and you can pass all the regular `FooterProps` to it.

### Dependencies

The `<Footer>` container has support for i18n out of the box if you include the [Translation](https://github.com/unicial/unicial-dapps#translation) module.
...
If you are using the [Translation](https://github.com/unicial/unicial-dapps#translation) module, the `Footer` contatiner comes with support for the 6 languages supported by the library.
...
The `<SignInPage>` container can be used in the same way as the `<SignIn>` component from [unicial-ui](https://github.com/unicial/ui) but it's already connected to the redux store. You can override any `SignInProp` if you want to connect differently, and you can pass all the regular `SignInProps` to it.
...
import SignInPage from 'unicial-dapps/dist/containers/SignInPage'
...
If you are using the [Translation](https://github.com/unicial/unicial-dapps#translation) module, the `SignInPage` contatiner comes with support for the 6 languages supported by the library.
...
The `<Modal>` it's a shorthand for some common features used by modals provided to [ModalProvider](https://github.com/unicial/unicial-dapps#modal).
...
export default class MyComponent extends React.PureComponent {
  render() {
    return (
      ...
      <Modal name={name} {/* Other Modal props from unicial ui */>
        <Modal.Header>
        </Modal.Header>
        <Modal.Description>
        </Modal.Description>
...
This container requires you to install the [Wallet](https://github.com/unicial/unicial-dapps#wallet) module

### Usage

```tsx
import * as React from 'react'
import TransactionLink from 'unicial-dapps/dist/containers/TransactionLink'
